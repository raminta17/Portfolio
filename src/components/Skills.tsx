...
import { softSkills, techSkills } from '../modules/data.tsx'
...
const Skills = () => {
	...
	const { ref: techSkillRef, inView: isTechSkillInView } = useInView({
		triggerOnce: true,
	})
	const { ref: softSkillRef, inView: isSoftSkillInView } = useInView({
		triggerOnce: true,
	})
	const {
		setIsHomeInView,
	...
	return (
		<div id='Skills' className='py-5'>
			<div
				className='content mx-auto p-3 d-flex flex-column gap-5 overflow-hidden'
				ref={skillsRef}
			>
				<h2 className='bottomBorder pb-3'>SKILLS</h2>
				<div className='skills md-column d-flex gap-5 mt-2'>
					<div className='box' ref={techSkillRef} style={styleTechSkill}>
						...
						<div className='py-3 techSkills gap-4'>
							{techSkills.map((skill, index) => (
								...
								>
									<img src={skill.link} alt='skill'></img>
									{skill.title}
								</div>
							))}
						</div>
					</div>
					<div className='box ' ref={softSkillRef} style={styleSoftSkill}>
